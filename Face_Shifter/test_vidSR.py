...
'''
target_frames_path = "/content/drive/MyDrive/FORMS_engine_1/Face_Shifter/dataset/luma2_src"
result_frames_path = "/content/drive/MyDrive/FORMS_engine_1/Face_Shifter/dataset/luma2_src_SR"

frames = glob.glob(os.path.join(target_frames_path,'*.*g'))
frames.sort()
ind = 0

for frame in frames:
    ind += 1
    img = utils.read_cv2_img(frame)
    print(img.shape)
    img = cv2.resize(img, dsize=(0, 0), fx=0.5, fy=0.5, interpolation=cv2.INTER_AREA)
    output_frame, rec_frame = sr_forward(img)
    utils.save_image(output_frame, '%08d.jpg'%ind)
    save_path = os.path.join(result_frames_path,'%08d.jpg'%ind)
    cv2.imwrite(save_path, output_frame)
    print('%08d.jpg'%ind, 'has been saved')
'''
img_path = '/content/drive/MyDrive/FORMS_engine_1/Face_Shifter/dataset/luma2_src/temp/temp.jpg'
img = utils.read_cv2_img(img_path)
output_img, rec_img = sr_forward(img)
utils.save_image(output_img, '/content/drive/MyDrive/FORMS_engine_1/Face_Shifter/dataset/luma2_src/temp/temp_SR.jpg')